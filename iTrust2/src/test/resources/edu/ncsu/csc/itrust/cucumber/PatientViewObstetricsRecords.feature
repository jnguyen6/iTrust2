Feature: Patient View Obstetrics Records
  ...
  Scenario Outline: View obstetrics record as a Patient who only has a past pregnancy. 
    ...
    When I navigate to the document obstetrics record page to document an obstetrics record for a patient.
    And I select radio button with text <username> to select a patient.
    And I enter <conception_year>, <weeks_pregnant>, <hours_in_labor>, <delivery_method>, <twins> for a previous obstetrics record to document an obstetrics record for a patient.
    And I click add to document an obstetrics record for a patient.
    Then I log out as OB/GYN HCP.
    Then I log on as a patient.
    When I navigate to the View Obstetrics Records page.
    Then I can view the previous obstetrics record <conception_year>, <weeks_pregnant>, <hours_in_labor>, <delivery_method>, <twins>.
    Then <text> is displayed on the page.  

    Examples: 
    | username      | conception_year | weeks_pregnant| hours_in_labor | delivery_method | twins | text                                      |
    | JillBob       | 2015            | 30            | 15             | Cesarean        | false | There are no current obstetrics records.  |
  ...
  Scenario Outline: View obstetrics record as a Patient who has current and past pregnancies.
    ...
    Examples:
    | username       | LMP         | dueDate    | weeksPreg    | conception_year | weeks_pregnant| hours_in_labor | delivery_method | twins |
    | JillBob        | 02/14/2017  | 09/21/2017 |   111        | 2015            | 30            | 15             | Cesarean        | false |

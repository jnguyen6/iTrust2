...
Scenario Outline: Patient has a previous but no current obstetrics records.
	Given A female patient exists in the system.
	Given We have added an HCP into the system.
	When As an OB/GYN HCP I add a previous obstetrics record for <username> with LMP <prev_lmp>, conception year <conception_year>, <weeks_pregnant> weeks pregnant, <hours_in_labor> hours in labor, <delivery_method>, twins: <twins> for a previous obstetrics record.
	Then I log in as an HCP and navigate to the View Obstetrics Records page.
	And I select to view <username>'s obstetrics records.
	Then The patient's correct LMP <prev_lmp>, conception year <conception_year>, <weeks_pregnant> weeks pregnant, <hours_in_labor> hours in labor, <delivery_method>, twins: <twins> for a previous obstetrics record are displayed for record <recordNo>.
	And The other user <username2> still has no records.
	
Examples:
	| username			| username2			| prev_lmp		| conception_year | weeks_pregnant| hours_in_labor | delivery_method | twins | recordNo	|
	| AliceThirteen	|	JillBob				| 03/12/2015	| 2015            | 36            | 10             | Vaginal         | true  | 0				|
	| JillBob				| AliceThirteen	| 11/26/2015	| 2015            | 30            | 15             | Cesarean        | false | 0				|
...
Examples:
	| username			| username2			| lmp					| prev_lmp		| conception_year | weeks_pregnant| hours_in_labor | delivery_method | twins | recordNo	|
	| AliceThirteen	|	JillBob				| 11/01/2018	| 03/12/2015	| 2015            | 36            | 10             | Vaginal         | true  | 1				|
	| JillBob				| AliceThirteen	| 04/03/2019	| 11/26/2015	| 2015            | 30            | 15             | Cesarean        | false | 1				|

...
public class APIObstetricsOfficeVisitController extends APIController {
    ...
    public ResponseEntity getObstetricsOfficeVisit ( @PathVariable ( "id" ) final Long id ) {
        final ObstetricsOfficeVisit visit = (ObstetricsOfficeVisit) ObstetricsOfficeVisit.getById(id);
        if ( null == visit ) {
            ...
            if ( null != self && self.isDoctor() ) {
                LoggerUtil.log( TransactionType.PATIENT_VIEWS_OBGYN_OFFICEVISIT, LoggerUtil.currentUser(),
    ...
    public ResponseEntity editObstetricsOfficeVisits ( @PathVariable final Long id,
        ...
        try {final ObstetricsOfficeVisit visit = new ObstetricsOfficeVisit( form );
            ...
            final ObstetricsOfficeVisit oVisit = (ObstetricsOfficeVisit) ObstetricsOfficeVisit.getById( id );
            if ( null == oVisit ) {
            ...
            if ( !oVisit.getBasicHealthMetrics().equals( visit.getBasicHealthMetrics() ) ) {
                LoggerUtil.log( TransactionType.EDIT_OBSTETRICS_RECORD, form.getHcp(), form.getPatient(), form.getHcp()
